...
#   bash capstone/validation/prepare-capstone.sh --build     also build 2.0.0 images
#
...
source "$R/VERSIONS.env"; source "$R/scripts/validate/_lib.sh"
...
if [[ $BUILD -eq 1 ]]; then
  ...
  ( eval "$(minikube -p "${MINIKUBE_PROFILE}" docker-env)" && \
    IMAGE_TAG="${IMAGE_TAG_V2}" bash "$R/scripts/build/build-all.sh" ) >/dev/null 2>&1 \
    && echo "  build finished" || echo "  build reported errors — see scripts/build/build-all.sh"
fi
...
K get secret axispay-tls -n "${NS_EDGE}" >/dev/null 2>&1 \
  ...
  || fail "axispay-tls missing" "the certificate INC-7 expires" "bash scripts/setup/06-generate-tls.sh"
...
for s in check-manifests.py simulate-netpol.py simulate-rbac.py check-helm-chart.py check-promql.py; do
  python3 "$R/scripts/validate/$s" >/dev/null 2>&1 && pass "$s" \
    || fail "$s reports failures" "all assertions holding" "python3 scripts/validate/$s"
done

echo
printf "%sInjection commands — keep this terminal open:%s\n" "$BLD" "$RST"
cat <<EOF
  00:40  bash scripts/incidents/inject-INC-5.sh    # redis -> 0
  00:52  bash scripts/incidents/inject-INC-6.sh    # data-tier policy narrowed  (THE TRAP)
  01:04  bash scripts/incidents/inject-INC-7.sh    # TLS certificate expired
