...
# ==============================================================================
#   bash platform/admin/validate/checkpoint.sh 3
#   bash platform/admin/validate/checkpoint.sh 3 --wait      wait for rollouts first
#
...
LAB_ID="DAY ${DAY} CHECKPOINT"; LAB_DOC="platform/reference/03-LAB-ROADMAP.md"
...
python3 "$D/simulate-netpol.py" >/dev/null 2>&1 \
  ...
  || fail "policy simulation reports failures" "all assertions holding" "python3 platform/admin/validate/simulate-netpol.py"
}
...
python3 "$D/simulate-rbac.py"       >/dev/null 2>&1 && pass "RBAC assertions hold"   || fail "RBAC simulation fails" "28 assertions" "python3 platform/admin/validate/simulate-rbac.py"
python3 "$D/check-helm-chart.py"    >/dev/null 2>&1 && pass "chart assertions hold"  || fail "chart validation fails" "94 assertions" "python3 platform/admin/validate/check-helm-chart.py"
python3 "$D/check-promql.py"        >/dev/null 2>&1 && pass "PromQL assertions hold" || fail "PromQL validation fails" "all expressions valid" "python3 platform/admin/validate/check-promql.py"
