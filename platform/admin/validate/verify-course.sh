...
#   bash platform/admin/validate/verify-course.sh --inventory   counts only, no checks
#
...
objs = 0
for f in glob.glob('platform/manifests/**/*.yaml', recursive=True) + glob.glob('capstone/manifests/*.yaml'):
    try: objs += sum(1 for d in yaml.safe_load_all(open(f)) if d)
    except Exception: pass
alerts = 0
for f in glob.glob('platform/manifests/day5/observability/02-prometheusrules.yaml'):
    for d in yaml.safe_load_all(open(f)):
        if d and d.get('kind') == 'PrometheusRule':
            alerts += sum(len(g['rules']) for g in d['spec']['groups'])
panels = 0
for f in glob.glob('platform/manifests/day5/observability/04-grafana-dashboards.yaml'):
    for d in yaml.safe_load_all(open(f)):
...
rows = [
    ("days",                 5),
    ("modules",              c('days/day*/labs/L*.md') + c('days/day*/labs/L*/README.md')),
    ("labs",                 c('days/day*/labs/L*.md') + c('days/day*/labs/L*/README.md')),
    ("incident windows",     c('days/day*/labs/INC*.md') + c('days/day*/labs/INC*/README.md') + 3),
    ("services",             len([d for d in glob.glob('platform/images/*') if os.path.isdir(d) and '_shared' not in d])),
    ("manifest files",       c('platform/manifests/**/*.yaml')),
    ("manifest objects",     objs),
    ("Helm chart templates", c('platform/charts/axispay/templates/*')),
    ("values files",         c('platform/charts/axispay/values*.yaml')),
    ("alert rules",          alerts),
    ("dashboard panels",     panels),
    ("mermaid diagrams",     c('platform/admin/authoring/diagrams/mermaid/*.mmd')),
    ("slide decks",          c('days/day*/AxisPay-K8s-Day*.pptx')),
    ("participant manuals",  c('days/day*/AxisPay-K8s-Day*-Participant-Manual.pdf')),
    ("trainer guides",       c('days/day*/trainer-guide.md')),
    ("assessments",          c('days/day*/assessment.md') + c('platform/reference/final-exam.md')),
    ("answer keys",          c('days/day*/answer-key.md') + c('platform/reference/final-exam-answer-key.md')),
    ("validators",           c('platform/admin/validate/*.sh') + c('platform/admin/validate/*.py')),
    ("scripts total",        c('platform/scripts/**/*.sh') + c('platform/scripts/**/*.py')
                             + c('platform/admin/**/*.sh') + c('platform/admin/**/*.py')),
    ("design docs",          c('platform/reference/*.md')),
]
for k, v in rows:
    print(f"  {k:22} {v}")
PY
...
while read -r f; do bash -n "$f" 2>/dev/null || { bad "shell syntax: $f"; BADSH=1; }; done \
  < <(find platform/scripts platform/admin capstone -name '*.sh' 2>/dev/null)
...
while read -r f; do python3 -m py_compile "$f" 2>/dev/null || { bad "python syntax: $f"; BADPY=1; }; done \
  < <(find platform/scripts platform/admin platform/images platform/data -name '*.py' 2>/dev/null)
[[ $BADPY -eq 0 ]] && ok "every Python file compiles"

python3 - <<'PY' && ok "every YAML file parses" || bad "a YAML file does not parse"
import yaml, glob, sys
bad = []
for f in (glob.glob('platform/manifests/**/*.yaml', recursive=True)
          + glob.glob('capstone/**/*.yaml', recursive=True)
          + glob.glob('platform/charts/**/values*.yaml', recursive=True)
          + ['platform/charts/axispay/Chart.yaml']):
...
if command -v node >/dev/null 2>&1; then
  while read -r f; do node --check "$f" >/dev/null 2>&1 || { bad "JS syntax: $f"; BADJS=1; }; done \
    < <(find platform/admin/authoring/slides-src -name '*.js' 2>/dev/null)
...
for s in check-manifests.py simulate-netpol.py simulate-rbac.py \
  ...
  if python3 "$D/$s" >/dev/null 2>&1; then
    ...
    bad "$s reports failures" "python3 platform/admin/validate/$s"
  fi
done
...
# ==============================================================================
MISSING=""
for f in days/day*/labs/L*.md days/day*/labs/L*/README.md; do
  [[ -e "$f" ]] || continue
  case "$f" in
    */README.md) id="$(basename "$(dirname "$f")" | cut -d- -f1)" ;;
    *)           id="$(basename "$f" | cut -d- -f1)" ;;
  esac
  [[ -f "platform/admin/validate/validate-lab-${id}.sh" ]] || MISSING="$MISSING $id"
done
[[ -z "${MISSING// /}" ]] && ok "every lab has a validator" \
  || bad "labs without a validator:$MISSING" "write platform/admin/validate/validate-lab-<ID>.sh"

ORPHAN=""
for f in platform/admin/validate/validate-lab-*.sh; do
  id="$(basename "$f" .sh | sed 's/validate-lab-//')"
  ls days/day*/labs/"${id}"-*.md >/dev/null 2>&1 && continue
  ls -d days/day*/labs/"${id}"-*/ >/dev/null 2>&1 && continue
  ORPHAN="$ORPHAN $id"
done
...
for d in 1 2 3 4 5; do
  MISS=""
  [[ -f "days/day$d/AxisPay-K8s-Day$d.pptx" ]]                        || MISS="$MISS deck"
  [[ -f "days/day$d/AxisPay-K8s-Day$d-Participant-Manual.pdf" ]]      || MISS="$MISS manual"
  [[ -f "days/day$d/trainer-guide.md" ]]                             || MISS="$MISS trainer-guide"
  [[ -f "days/day$d/assessment.md" ]]                                || MISS="$MISS assessment"
  [[ -f "days/day$d/answer-key.md" ]]                                || MISS="$MISS answer-key"
  [[ -f "days/day$d/README.md" ]]                                    || MISS="$MISS readme"
  [[ -f "days/day$d/manual-chapter.md" ]]                            || MISS="$MISS manual-source"
  [[ -f "days/day$d/solutions.md" ]]                                 || MISS="$MISS solutions"
  [[ -f "platform/admin/validate/checkpoint-day$d.sh" ]]           || MISS="$MISS checkpoint"
  { ls days/day$d/labs/L*.md >/dev/null 2>&1 || ls -d days/day$d/labs/L*/ >/dev/null 2>&1; } || MISS="$MISS labs"
  ls platform/admin/authoring/diagrams/mermaid/d$d-*.mmd >/dev/null 2>&1  || MISS="$MISS diagrams"
...
while read -r f; do
  ...
  if [[ -n "${MISS// /}" ]]; then
    bad "$(dirname "$f" | sed 's|days/day[0-9]/labs/||') missing:$MISS" "see days/day1/labs/L1.3-first-pod/README.md for the shape"
    FMT=1
  fi
done < <(find days -path '*/labs/*' -name README.md -mindepth 4 | sort)
...
# never leaves the folder. platform/manifests/ remains the canonical set that
...
while read -r f; do
  ...
  # ones with the same filename — day2/resources/ replaces day1/deployments/ —
  # so a bare basename search finds the wrong file.
  day="$(echo "$f" | sed -n 's|^days/\(day[0-9]\)/.*|\1|p')"
  src="$(find "platform/manifests/$day" platform/manifests/00-namespaces -name "$b" -type f 2>/dev/null | head -1)"
  [[ -z "$src" ]] && src="$(find platform/manifests -name "$b" -type f | head -1)"
  LCOUNT=$((LCOUNT+1))
  if [[ -z "$src" ]]; then
    bad "$f has no counterpart in platform/manifests/" "every lab manifest must be a copy of a canonical one"
    LDRIFT=1
  elif ! cmp -s "$f" "$src"; then
    bad "$f differs from $src" "diff \"$f\" \"$src\"   # then copy the canonical one over it"
    LDRIFT=1
  fi
done < <(find days -path '*/labs/*/manifests/*.yaml' 2>/dev/null | sort)
[[ $LDRIFT -eq 0 ]] && ok "all $LCOUNT lab manifest copies match platform/manifests/"
...
missing = []
pat = re.compile(r'(?:platform/manifests|platform/charts|platform/scripts|platform/admin|platform/images|platform/data|manifests|charts|scripts|admin|images|data|capstone)/[A-Za-z0-9_./-]+')
for f in glob.glob('days/**/*.md', recursive=True) + glob.glob('capstone/**/*.md', recursive=True):
    for m in pat.findall(open(f, errors='ignore').read()):
        p = m.rstrip('.,;:)')
        if any(ch in p for ch in '<>*$'): continue
        # A token resolves if it (or its parent dir) exists at the repo root or
        # under platform/. Lab folders hold their own manifests/ copies, so a
        # bare "manifests/NN-x.yaml" is satisfied by platform/manifests existing.
        cands = [p, os.path.dirname(p),
                 os.path.join('platform', p),
                 os.path.join('platform', os.path.dirname(p))]
        if not any(os.path.exists(c) for c in cands if c):
...
PAN=$(grep -rEc '\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13})\b' \
      platform/data/seed/*.sql 2>/dev/null | awk -F: '{s+=$2} END {print s+0}')
...
REAL=$(grep -rniE '\b(visa|mastercard|stripe|adyen|paypal) (inc|ltd|plc|corp)\b' \
       platform/data/ platform/images/ platform/manifests/ 2>/dev/null | wc -l)
...
chart = yaml.safe_load(open('platform/charts/axispay/Chart.yaml'))
vals = yaml.safe_load(open('platform/charts/axispay/values.yaml'))
problems = []
