...
Build a participant-manual PDF from Markdown.
    ...
    python3 platform/scripts/build/build_manual.py days/day1/manual-chapter.md \
...
def build(src: pathlib.Path, out: pathlib.Path, title: str) -> None:
    ...
    # Mermaid blocks are rendered separately into platform/diagrams/png; in the PDF they
    # become a labelled reference rather than an unreadable wall of source.
    text = re.sub(
        r"```mermaid\n(.*?)```",
        lambda m: "> **Diagram** — see `platform/diagrams/png/` for the rendered version.\n",
        text, flags=re.S,
    )
