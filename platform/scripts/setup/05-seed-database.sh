...
K -n "${NS_DATA}" exec -i postgres-0 -- \
  psql -U axispay_app -d axispay -v ON_ERROR_STOP=1 < "$R/platform/data/schema/01-schema.sql"

echo "Loading seed data (~28,000 statements, this takes 30-60s)..."
K -n "${NS_DATA}" exec -i postgres-0 -- \
  psql -U axispay_app -d axispay -q -v ON_ERROR_STOP=1 < "$R/platform/data/seed/02-seed.sql"
