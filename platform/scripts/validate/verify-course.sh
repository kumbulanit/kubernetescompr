...
rows = [
    ...
    ("mermaid diagrams",     c('platform/diagrams/mermaid/*.mmd')),
    ...
    ("validators",           c('platform/scripts/validate/*.sh') + c('platform/scripts/validate/*.py')),
...
while read -r f; do bash -n "$f" 2>/dev/null || { bad "shell syntax: $f"; BADSH=1; }; done \
  < <(find platform/scripts capstone platform/diagrams -name '*.sh' 2>/dev/null)
...
while read -r f; do python3 -m py_compile "$f" 2>/dev/null || { bad "python syntax: $f"; BADPY=1; }; done \
  < <(find platform/scripts platform/images platform/data -name '*.py' 2>/dev/null)
...
if command -v node >/dev/null 2>&1; then
  while read -r f; do node --check "$f" >/dev/null 2>&1 || { bad "JS syntax: $f"; BADJS=1; }; done \
    < <(find platform/slides-src -name '*.js' 2>/dev/null)
...
for s in check-manifests.py simulate-netpol.py simulate-rbac.py \
  ...
  if python3 "$D/$s" >/dev/null 2>&1; then
    ...
    bad "$s reports failures" "python3 platform/scripts/validate/$s"
  fi
done
...
for f in days/day*/labs/L*.md days/day*/labs/L*/README.md; do
  ...
  [[ -f "platform/scripts/validate/validate-lab-${id}.sh" ]] || MISSING="$MISSING $id"
done
[[ -z "${MISSING// /}" ]] && ok "every lab has a validator" \
  || bad "labs without a validator:$MISSING" "write platform/scripts/validate/validate-lab-<ID>.sh"

ORPHAN=""
for f in platform/scripts/validate/validate-lab-*.sh; do
...
for d in 1 2 3 4 5; do
  ...
  [[ -f "platform/scripts/validate/checkpoint-day$d.sh" ]]           || MISS="$MISS checkpoint"
  { ls days/day$d/labs/L*.md >/dev/null 2>&1 || ls -d days/day$d/labs/L*/ >/dev/null 2>&1; } || MISS="$MISS labs"
  ls platform/diagrams/mermaid/d$d-*.mmd >/dev/null 2>&1             || MISS="$MISS diagrams"
...
missing = []
pat = re.compile(r'(?:platform/manifests|platform/charts|platform/scripts|platform/images|platform/data|manifests|charts|scripts|images|data|capstone)/[A-Za-z0-9_./-]+')
