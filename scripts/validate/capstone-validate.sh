...
#   bash scripts/validate/capstone-validate.sh --brief     one line per section
#
...
# against documents/instructor/capstone-rubric.md. This script checks the OUTCOME.
#
...
if K get netpol allow-core-and-async-to-data -n "${NS_DATA}" >/dev/null 2>&1; then
  ...
  fail "data-tier NetworkPolicy IS MISSING" \
       ...
       segmentation. Restore with:
         kubectl apply -f manifests/day4/netpol/05-data-tier.yaml
       See documents/instructor/capstone-rubric.md §3 (the trap)."
fi
