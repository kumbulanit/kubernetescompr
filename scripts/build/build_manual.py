...
Build a participant-manual PDF from Markdown.
    ...
    python3 scripts/build/build_manual.py topics/01-foundations-and-core-objects/manual-chapter.md \
        -o documents/manuals/AxisPay-K8s-Day1-Participant-Manual.pdf
...
def build(src: pathlib.Path, out: pathlib.Path, title: str) -> None:
    ...
    # Mermaid blocks are rendered separately into diagrams/png; in the PDF they
    # become a labelled reference rather than an unreadable wall of source.
    text = re.sub(
        r"```mermaid\n(.*?)```",
        lambda m: "> **Diagram** — see `diagrams/png/` for the rendered version.\n",
        text, flags=re.S,
    )
